...
class SemanticQAGen:
    ...
    def process_input_directory(self, output_format: Optional[str] = None) -> Dict[str, Any]:
        ...
        for filename in files_in_dir:
            ...
            try:
                ...
                self.logger.exception(f"  Unexpected critical error processing {filename}: {unexpected_err}",
                                      exc_info=True)
                failed_count += 1
            ...
            for q in questions:
                if isinstance(q, BaseModel):  # Check if using Pydantic Question model
                    try:
                        ...
                        self.logger.warning(
                            f"Failed to dump Question model {getattr(q, 'id', '?')}: {dump_err}. Skipping.")
    ...
    def dump_failed_chunks(self, output_path: Optional[str] = None) -> int:
        ...
        Args:
            output_path: Optional path to write the report. If None, logs to logger only.

        Returns:
            Number of failed chunks reported
        """
        if not hasattr(self.pipeline, 'failed_chunks') or not self.pipeline.failed_chunks:
            self.logger.info("No failed chunks to report.")
            return 0

        failed_chunks = self.pipeline.failed_chunks
        report_lines = [f"=== FAILED CHUNKS REPORT ({len(failed_chunks)} chunks) ==="]

        for i, (chunk_id, info) in enumerate(failed_chunks.items(), 1):
            report_lines.append(f"\n\n== FAILED CHUNK #{i}: {chunk_id} ==")

            # Content preview
            content = info.get('content', 'Content not available')
            report_lines.append(f"\nCONTENT PREVIEW:\n{content}")

            # Errors section
            errors = info.get('errors', ['Unknown error'])
            report_lines.append(f"\nERRORS ({len(errors)}):")
            for j, error in enumerate(errors, 1):
                report_lines.append(f"  {j}. {error}")
            ...
            if details:
                ...
                for key, value in details.items():
                    if isinstance(value, dict):
                        ...
                        if len(v_str) > 500:
                            v_str = v_str[:500] + "..."
                        report_lines.append(f"  {key}: {v_str}")
        ...
        if output_path:
            ...
            with open(output_path, 'w', encoding='utf-8') as f:
                f.write(report_text)
            self.logger.info(f"Failed chunks report written to {output_path}")
        else:
            # Only log high-level summary if not writing to file
            self.logger.info(f"Failed chunks summary: {len(failed_chunks)} chunks failed")
            for chunk_id, info in failed_chunks.items():
                errors = info.get('errors', ['Unknown error'])
                self.logger.info(f"  - Chunk {chunk_id}: {errors[0]}")
