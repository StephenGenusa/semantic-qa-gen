...
class PDFLoaderConfig(LoaderConfig):
    ...
    use_advanced_reading_order: bool = Field(False,
                                             description="Use advanced reading order algorithm (better for complex layouts)")
...
class ChunkingConfig(BaseModel):
    ...
    overlap_size: int = Field(150, ge=0,
                              description="Size of overlap between chunks (chars/tokens depend on strategy).")
    preserve_headings: bool = Field(True,
                                    description="Whether to attempt splitting chunks before major headings (semantic strategy).")
...
class LocalServiceConfig(BaseLLMServiceDetails):
    ...
    model: str = Field("mistral:7b",
                       description="Default model name for the local service (e.g., 'mistral:7b', 'llama3:8b').")
    url: HttpUrl = Field(
        "http://localhost:11434",
        description="Base URL for the local LLM API (e.g., Ollama base /v1 endpoint, LM Studio OpenAI-compatible endpoint)."
    )
...
class RemoteServiceConfig(BaseLLMServiceDetails):
    ...
    model: str = Field("gpt-4o",
                       description="Default model name recognized by the remote API provider (e.g., 'gpt-4o', 'claude-3-opus-20240229').")
    provider: str = Field(
        "openai",
        description="Helps select adapter logic or API specifics ('openai', 'azure', 'openrouter', 'anthropic', etc.)."
    )
    api_base: Optional[HttpUrl] = Field(
        None,
        description="API base URL. Often needed for non-standard providers or Azure. Can sometimes be inferred if provider is known (e.g., default OpenAI API base)."
    )
    preferred_tasks: List[str] = Field(
    ...
    def check_model_recommendation(cls, v: str, info: ValidationInfo):  # Pydantic v2 passes ValidationInfo
        ...
        if is_openai_provider and not ('gpt-4' in v or 'gpt-3.5' in v):
            warnings.warn(
                f"Model '{v}' specified for OpenAI provider. Consider using standard models like 'gpt-4o', 'gpt-4-turbo', or 'gpt-3.5-turbo' unless using fine-tuned models.",
                UserWarning
            )
        return v
    ...
    def validate_llm_config(self) -> 'LLMServiceConfig':
        ...
        if not local_enabled and not remote_enabled:
            ...
            if local_config is None and remote_config is None:
                warnings.warn(
                    "No LLM services ('local' or 'remote') are defined in the configuration. LLM-dependent features will fail.",
                    UserWarning)
            else:
        ...
        if remote_enabled and not getattr(remote_config, 'api_key', None):
            warnings.warn(
                "config: Remote LLM service is enabled, but no api_key is set. Ensure it's provided via environment variable or config.",
                UserWarning)
...
class QuestionGenerationConfig(BaseModel):
    ...
    adaptive_generation: bool = Field(True,
                                      description="Enable adaptive question count based on chunk analysis (density).")
...
class DiversityValidatorConfig(BaseValidatorConfig):
    threshold: float = Field(0.85, ge=0.0, le=1.0,
                             description="Similarity threshold (e.g., SequenceMatcher ratio) ABOTE which questions are rejected by this validator.")
...
class OutputConfig(BaseModel):
    ...
    include_statistics: bool = Field(True,
                                     description="Include processing statistics in output file (may be separate file for CSV).")
    output_dir: DirectoryPath = Field("./output", description="Directory to save output files.")

    # Add fine tuning format options
    fine_tuning_format: str = Field("default",
                                    description="Field mapping format for AI fine-tuning ('default', 'openai_chat', 'openai_legacy', 'standard').")
    ...
    csv_include_document_info: bool = Field(False,
                                            description="Include document metadata columns in CSV output (can make CSV large).")

    @field_validator("format")
    @classmethod
    def validate_format(cls, v: str):
        valid_formats = ["json", "csv", "jsonl"]  # Add more as implemented
        if v.lower() not in valid_formats:
            raise ValueError(f"Output format must be one of {valid_formats}")
        return v.lower()

    @field_validator("fine_tuning_format")
    @classmethod
    def validate_fine_tuning_format(cls, v: str):
        valid_formats = ["default", "openai_chat", "openai_legacy", "standard"]
        if v.lower() not in valid_formats:
            raise ValueError(f"Fine-tuning format must be one of {valid_formats}")
        return v.lower()
...
class ProcessingConfig(BaseModel):
    ...
    debug_mode: bool = Field(False,
                             description="Enable debug mode (more verbose logging, potentially saves failed LLM inputs/outputs).")
